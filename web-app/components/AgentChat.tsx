...
export function AgentChat() {
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="relative">
                            ...
                            <div className="relative flex justify-center text-xs">
                                <span className="px-2 bg-[#111111] text-zinc-500">or try an example</span>
                            </div>
                        </div>
                        <button
                            onClick={() => setWallet('7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU')}
                            className="w-full text-xs text-zinc-400 hover:text-white transition-colors font-mono p-2 rounded-lg hover:bg-white/5"
                        >
    ...
    return (
        <Card padding="none" className="max-w-2xl mx-auto overflow-hidden shadow-xl">
            ...
            <div className="relative px-6 py-4 border-b border-white/10 bg-gradient-to-r from-[#FAFAF8] to-white">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-semibold text-white">Solana Agent</p>
                            <p className="text-xs text-[#14B8A6]">Live Data via sly_data</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                        <span className="text-xs font-mono bg-white/5 px-3 py-1.5 rounded-lg text-zinc-400">
            ...
            <div className="h-96 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-[#FAFAF8]/50 to-white">
                    ...
                    >
                        <div
                            className={`max-w-[85%] rounded-2xl px-5 py-3.5 ${message.role === 'user'
                                ...
                                : 'bg-[#111111] text-white rounded-bl-md shadow-md border border-white/10'
                                }`}
                        >
                            <div
                                ...
                                dangerouslySetInnerHTML={{
                                    __html: message.content
                                        ...
                                        .replace(/`(.*?)`/g, `<code class="${message.role === 'user' ? 'bg-[#111111]/20' : 'bg-white/5'} px-1.5 py-0.5 rounded text-xs font-mono">$1</code>`)
                                        .replace(/\n/g, '<br>')
                                }}
                            />
                        </div>
                ...
                {isLoading && (
                    <div className="flex justify-start animate-fade-in">
                        <div className="bg-[#111111] rounded-2xl rounded-bl-md px-5 py-4 shadow-md border border-white/10">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm text-zinc-400">Querying Solana...</span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                    {['What is my balance?', 'Show my tokens', 'Recent transactions'].map((suggestion) => (
                        <button
                            ...
                            className="text-xs text-zinc-400 px-3 py-1.5 rounded-full border border-white/10 hover:border-[#14B8A6] hover:text-[#14B8A6] transition-colors"
                        >
                            {suggestion}
                        </button>
                    ))}
